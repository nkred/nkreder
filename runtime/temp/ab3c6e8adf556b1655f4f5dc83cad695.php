<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:32:"../Theme/admin/report\index.html";i:1557808128;s:42:"E:\php\loan\Theme\admin\Public\header.html";i:1554530736;s:42:"E:\php\loan\Theme\admin\Public\footer.html";i:1534405538;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
<meta charset="utf-8">
		...
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<?php foreach($data as $li): ?>
				<tr class="text-c">
					...
					<td>
						<?php if($li['is_report']==1): ?>
						<span class="label label-success radius">已报到</span>
						<?php else: ?>
						<span class="label radius">未报到</span>
						<?php endif; ?>
					</td>
					<td class="f-14">
						<?php if($li['is_report']==0): ?>
							<a style="text-decoration:none" onClick="start('<?php echo $li['id']; ?>')" href="javascript:;" title="报到">
...
<script type="text/javascript">
    function start(id){
        layer.confirm('确认要报到吗？',function(index){
            $.ajax({
                type: 'Get',
                ...
                success: function(data){
                    res = JSON.parse(data);
                    if(res['status'] == 1){
                        layer.msg('报到成功',{icon:1,time:1000},function () {
                            location.reload();
                        });
                    }else {
                        layer.msg('无法报到',{icon:1,time:1000},function () {
                            location.reload();
                        });
                    }
                },
